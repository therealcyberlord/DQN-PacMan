...
def epsilon_greedy(env, net, state, epsilon):
    ...
    if np.random.random() <= epsilon:
        ...
        with torch.no_grad():
            ...
            state = torch.tensor(state, dtype=torch.float32).view(-1, 4, 1, height, width)
            net_out = net(state)
...
def optimize_model(policy, target, replay_buffer, gamma, optim, criterion, batch_size):
    ...
    states = torch.tensor(states, dtype=torch.float32, device=device).view(-1, 4, 1, height, width)
    ...
    non_final_next_states = torch.tensor(non_final_next_states, dtype=torch.float32, device=device).view(-1, 4, 1, height, width)

...
class Train_DQN:
    ...
    def optimize_model(self, policy, target, replay_buffer, gamma, optim, criterion, batch_size):
        ...
        states = torch.tensor(states, dtype=torch.float32, device=device).view(-1, 1, self.height, self.width)
        ...
        non_final_next_states = torch.tensor(non_final_next_states, dtype=torch.float32, device=device).view(-1, 1, self.height, self.width)

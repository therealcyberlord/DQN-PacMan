...
from Configs import device , num_actions, height, width
...
class Train_DQN:
    ...
    def epsilon_greedy(self, state, epsilon):
        ...
        if np.random.random() <= epsilon:
            ...
            with torch.no_grad():
                ...
                state = torch.tensor(state, dtype=torch.float32, device=device).view(-1, height, width).unsqueeze(1)
    ...
    def optimize_model(self):
        ...
        states = torch.tensor(states, dtype=torch.float32, device=device).view(-1, height, width).unsqueeze(1)
        ...
        non_final_next_states = torch.tensor(non_final_next_states, dtype=torch.float32, device=device).view(-1, height, width).unsqueeze(1)
        ...
        with torch.no_grad():
            ...
            next_state_max_q_values = torch.zeros(self.batch_size // 4, device=device)

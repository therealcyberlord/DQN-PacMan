...
class Train_DQN:
    ...
    def epsilon_greedy(self, state, epsilon):
        ...
        if np.random.random() <= epsilon:
            ...
            with torch.no_grad():
                ...
                state = torch.tensor(state, dtype=torch.float32).view(self.batch_size, self.channel, self.height, self.width)
    ...
    def optimize_model(self):
        ...
        states = torch.tensor(states, dtype=torch.float32, device=self.device).view(self.batch_size, 1, self.height, self.width)
        ...
        non_final_next_states = torch.tensor(non_final_next_states, dtype=torch.float32, device = self.device).view(self.batch_size, 1, self.height, self.width)
    ...
    def learn(self, num_episodes, max_episode_steps, min_samples_for_training, train_period, update_target_net_steps, end_epsilion_decay):
        for episode in range(num_episodes):
            ...
            self.logger.print_stats(episode+1, episode_reward, episode_steps, self.step_counter) 

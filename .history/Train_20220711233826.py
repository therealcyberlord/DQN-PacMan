...
class Train_DQN:
    ...
    def epsilon_greedy(self, state, epsilon):
        ...
        if np.random.random() <= epsilon:
            ...
            with torch.no_grad():
                ...
                state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0).view(-1, channel, height, width)
    ...
    def optimize_model(self):
        ...
        states = torch.tensor(states, dtype=torch.float32, device=self.device).unsqueeze(0).view(-1, channel, height, width)

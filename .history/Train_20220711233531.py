...
class Train_DQN:
    def __init__(self, env, height, width, batch_size, channel, replay_buffer, policy, target, gamma, optim, criterion, save_period):
        self.env = env 
        self.height = height
        self.width = width
    ...
    def epsilon_greedy(self, state, epsilon):
        ...
        if np.random.random() <= epsilon:
            ...
            with torch.no_grad():
                ...
                state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0).view(-1, 1, self.height, self.width)
    ...
    def optimize_model(self):
        ...
        states = torch.tensor(states, dtype=torch.float32, device=self.device).unsqueeze(0).view(-1, 1, self.height, self.width)
        ...
        non_final_next_states = torch.tensor(non_final_next_states, dtype=torch.float32, device = self.device).unsqueeze(1)

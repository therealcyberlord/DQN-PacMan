...
class Train_DQN:
    ...
    def learn(self, num_episodes, max_episode_steps, min_samples_for_training, train_period, update_target_net_steps, end_epsilon_decay):
        for episode in range(num_episodes):
            ...
            for step in range(max_episode_steps):
                ...
                action = self.epsilon_greedy(self.env, self.policy, state, max(1 - episode / 500, end_epsilion_decay))
                ...
                if len(self.replay_buffer) > min_samples_for_training:
                    if step % train_period == 0:
                        self.optimize_model(self.policy, self.target, self.replay_buffer, self.gamma, self.optimizer, self.criterion, self.batch_size)

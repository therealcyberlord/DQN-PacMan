...
class Train_DQN:
    ...
    def epsilon_greedy(self, state, epsilon):
        ...
        if np.random.random() <= epsilon:
            ...
            with torch.no_grad():
                ...
                state = torch.tensor(state, dtype=torch.float32, device=device).view(-1, height, width).unsqeeze(1)
    ...
    def optimize_model(self):
        ...
        non_final_next_states = torch.tensor(non_final_next_states, dtype=torch.float32, device=device).view(-1, height, width).unsqeeze(1)
